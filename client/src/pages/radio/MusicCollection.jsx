...
    const artistas = track.artistas_participantes && typeof track.artistas_participantes === 'string' 
                      ? JSON.parse(track.artistas_participantes) 
                      : []
    
    let dias = [];
    try {
      if (track.dias_semana && typeof track.dias_semana === 'string') {
        const parsedDays = JSON.parse(track.dias_semana);
        dias = Array.isArray(parsedDays) ? parsedDays : [];
      } else if (Array.isArray(track.dias_semana)) {
        dias = track.dias_semana;
      }
    } catch (e) {
      dias = [];
    }

    // Garante que o estado dias_semana seja sempre um array válido para o formulário
    setFormData({
      ...track,
      artistas_participantes: Array.isArray(artistas) ? artistas : [],
      dias_semana: dias.length > 0 ? dias : [...ALL_DAYS_ARRAY], 
      ano: track.ano || '' 
    })
...
          // Regra: Se já está em TODOS, não faz nada (no-op)
          newDays = [...currentDays];
        } else {
          // Clicou em TODOS: Define o array completo (7 tags)
          newDays = [...ALL_DAYS_ARRAY];
        }
      } else {
        // Lógica de botões individuais
        
        if (isCurrentlyAllSelected) {
          // Transição de TODOS: desmarca todos e seleciona apenas o dia clicado
          newDays = [index];
        } else if (newDays.includes(index)) {
          // Desmarcar: remove o dia se for possível (não é o último)
          if (newDays.length > 1) {
            newDays = newDays.filter(d => d !== index);
          } else {
            // Se for o último dia, NADA ACONTECE (Regra: estado vazio proibido)
            newDays = [...currentDays];
          }
        } else {
          // Marcar: adiciona o dia
          newDays.push(index);
        }
        
        // Regra: Se a seleção individual se tornar completa (7 dias), normaliza para o array TODOS.
        if (newDays.length === 7) {
          newDays = [...ALL_DAYS_ARRAY];
        }
      }
