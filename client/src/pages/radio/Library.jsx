...
    let trackIds = [];
    if (Array.isArray(playlist.tracks_ids)) {
      trackIds = playlist.tracks_ids.map(id => Number(id)).filter(id => !isNaN(id));
    } else {
      try {
        const parsed = JSON.parse(playlist.tracks_ids || '[]'); 
        if (Array.isArray(parsed)) {
          trackIds = parsed.map(id => Number(id)).filter(id => !isNaN(id)); 
        }
      } catch (e) {
        console.error(`Erro ao parsear tracks_ids da playlist ${playlist.id}:`, playlist.tracks_ids, e);
        trackIds = [];
      }
    }
...
      setError(`Falha ao excluir a playlist "${playlistToDelete.nome}".`);
      closeDeletePlaylistModal();
    }
  };
...
            </div>
            <button onClick={() => navigate('/radio/playlist-creator')} className="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-lg font-semibold hover:bg-primary/80 transition-all shadow-lg hover:scale-105">
...
                    style={playlist.imagem ? { backgroundImage: `url(${playlist.imagem})` } : {}}
                  >
                    {!playlist.imagem && <span className="material-symbols-outlined text-white text-6xl opacity-50">queue_music</span>}
                    <div className={`absolute inset-0 transition-opacity duration-300 ${playlist.imagem ? 'bg-black/40 group-hover:bg-black/60' : 'bg-black/20'}`}></div>
                    <button className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity z-10 p-1 rounded-full bg-black/30 hover:bg-black/50">
                      <span className="material-symbols-outlined text-white text-lg">more_vert</span>
                    </button>
                  </div>
                  <div className="p-5">
